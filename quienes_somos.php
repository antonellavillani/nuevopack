<?php
// Incluir el header
include('includes/header.php');
?>

<body>
    <!-- Contenido Principal -->
    <main class="articles">

        <!-- Título Principal -->
        <h1>Quiénes Somos</h1>
        <h2>Más de 20 años impulsando ideas creativas y únicas</h2>
        
        <!-- Sección: Nuestra Historia -->
        <section class="nuestra-historia">
            <h2>Nuestra Historia</h2>
            <p>En NuevoPack, llevamos más de 20 años ofreciendo soluciones de impresión innovadoras, 
        adaptándonos a las necesidades de nuestros clientes y comprometiéndonos con la calidad y la creatividad.</p>
            <img src="imagen_historia.jpg" alt="Foto de nuestra historia" class="imagen-historia">
        </section>

        <!-- Sección: Misión, Visión y Valores -->
        <section class="mision-vision-valores">
            <h2>Misión, Visión y Valores</h2>
            <div class="seccion mision">
                <h3>Misión</h3>
                <p>En NuevoPack, nuestra misión es ofrecer soluciones de impresión personalizadas que combinen creatividad, calidad e innovación. Nos esforzamos por superar las expectativas de nuestros clientes y convertir sus ideas en proyectos que marquen la diferencia.</p>
            </div>

            <div class="seccion vision">
                <h3>Visión</h3>
                <p>Ser líderes en el mercado de impresión, destacándonos por nuestra capacidad de innovar, nuestro compromiso con la sostenibilidad y nuestra pasión por el diseño creativo.</p>
            </div>
            <div class="seccion valores">
                <h3>Valores</h3>
                <ul>
                    <li><strong>Calidad:</strong> Ofrecemos productos que cumplen con los estándares más altos.</li>
                    <li><strong>Creatividad:</strong> Transformamos ideas en diseños únicos y funcionales.</li>
                    <li><strong>Innovación:</strong> Incorporamos nuevas tecnologías para mejorar continuamente.</li>
                    <li><strong>Sostenibilidad:</strong> Priorizamos prácticas responsables con el medio ambiente.</li>
                    <li><strong>Compromiso con el cliente:</strong> Escuchamos y entendemos las necesidades de cada proyecto.</li>
                </ul>
            </div>
        </section>

        <!-- Sección: Nuestro Equipo -->
        <section class="nuestro-equipo">
            <h2>Nuestro Equipo</h2>
            <p>Detrás de NuevoPack hay un equipo apasionado por la creatividad y la excelencia. Desde diseñadores hasta especialistas en impresión, cada miembro aporta su experiencia y compromiso para brindar soluciones que superen tus expectativas. Estamos aquí para acompañarte en cada paso de tu proyecto.</p>
            <img src="equipo.jpg" alt="Foto del equipo" class="imagen-equipo">
        </section>

        <!-- Sección: Logros y Datos Clave -->
        <section class="logros-datos">
            <h2>Logros y Datos Clave</h2>
            <ul>
                <li><strong>+1,500 proyectos entregados:</strong> Con soluciones que combinan calidad y creatividad.</li>
                <li><strong>Presencia en todo el país:</strong> Ampliando nuestro alcance gracias a nuestra plataforma online.</li>
                <li><strong>Compromiso sostenible:</strong> Implementando procesos que respetan el medio ambiente.</li>
                <li><strong>100% de clientes satisfechos:</strong> Gracias a nuestra atención personalizada y resultados excepcionales.</li>
            </ul>
        </section>

        <!-- Sección: Nuestro Compromiso con la Sostenibilidad -->
        <section class="sostenibilidad">
            <h2>Nuestro Compromiso con la Sostenibilidad</h2>
            <p>En NuevoPack, creemos que el futuro de la impresión está en la sostenibilidad. Por eso, trabajamos con materiales reciclables y tecnologías que minimizan el impacto ambiental. Cada paso que damos refleja nuestro compromiso con un mundo más responsable y consciente.</p>
        </section>

        <!-- Sección final: Cierre inspirador -->
        <section class="cierre">
            <p>En NuevoPack, no solo imprimimos, hacemos realidad tus ideas. Queremos ser tu socio de confianza en cada proyecto, grande o pequeño. ¡Contáctanos hoy y descubre cómo podemos ayudarte a crear algo extraordinario!</p>
            <div class="botones">
                <a href="contacto.html" class="btn">Contáctanos</a>
                <a href="catalogo.html" class="btn">Conoce nuestros productos</a>
            </div>
        </section>

    </main>

<?php
// Incluir el footer
include('includes/footer.php');
?>